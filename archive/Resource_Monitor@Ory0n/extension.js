        ...
        _refreshWlanValue() {
            this._loadFile('/proc/net/dev').then(contents => {
                ...
                if (delta > 0) {
                    ...
                    if (du[0] > unit || du[1] > unit) {
                        this._wlanUnit.text = boolUnit ? 'kb/s' : 'KB/s';
                        du[0] /= unit;
                        du[1] /= unit;
                        if (du[0] > unit || du[1] > unit) {
                            this._wlanUnit.text = boolUnit ? 'mb/s' : 'MB/s';
                            du[0] /= unit;
                            du[1] /= unit;
                            if (du[0] > unit || du[1] > unit) {
                                this._wlanUnit.text = boolUnit ? 'gb/s' : 'GB/s';
                                du[0] /= unit;
                                du[1] /= unit;
                            }
                        }
                    } else {
                        this._wlanUnit.text = boolUnit ? 'b/s' : 'B/s';
                    }
                }
...
class Extension {
    ...
    enable() {
        ...
        this._handlerId = this._settings.connect(`changed::${EXTENSION_POSITION}`, () => {
            ...
            Main.panel.addToStatusArea(this._uuid, this._indicator, 3, this._extensionPosition);
        });
